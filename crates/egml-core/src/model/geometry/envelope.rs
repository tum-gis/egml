...
impl Envelope {
    pub fn from_envelopes(envelopes: &Vec<Self>) -> Result<Self, Error> {
    ...
    pub fn from_optional_envelopes(
        envelopes: &Vec<Option<Envelope>>,
    ) -> Result<Option<Self>, Error> {
        let flattened_envelopes: Vec<Envelope> = envelopes.iter().cloned().flatten().collect();
        if flattened_envelopes.is_empty() {
            return Ok(None);
        }

        Self::from_envelopes(&flattened_envelopes).map(Some)
    }
}
